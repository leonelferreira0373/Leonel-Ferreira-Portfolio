...
export function Gallery() {
  ...
  return (
    <section id="gallery" className="py-16">
      ...
      >
        <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto px-[10%] py-2 [scroll-padding-inline:10%] [scrollbar-width:none] md:gap-5 md:px-[30%] md:[scroll-padding-inline:30%] [&::-webkit-scrollbar]:hidden">
          {items.map((g, i) => (
            <img
              key={g.src}
              src={g.src}
              ...
              decoding="async"
              className={`${aspectClass} w-[80%] flex-none snap-center rounded-2xl border border-border/60 bg-card object-cover shadow-lg shadow-black/5 md:w-[40%] dark:shadow-black/30`}
            />
          ))}
        </div>
      </div>
